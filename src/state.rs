...
impl State {
    ...
    pub fn layout(&mut self, size: glam::Vec2) {
        ...
        if old.x > 0.0 && old.y > 0.0 {
            ...
            self.player1
                .update_y_position(self.player1.position().y * size.y / old.y);
            self.player2
                .update_y_position(self.player2.position().y * size.y / old.y);
        ...
        self.player1
            .update_position((size.x * 0.1, self.player1.position().y).into());
        self.player2
            .update_position((size.x * 0.9, self.player2.position().y).into());

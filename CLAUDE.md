...
Two paddles, a ball, five points to win. Built on `dynamo_lib`, which lives at
`../rust/dynamo_lib` and owns the window, rendering, input, and sound.
...
- `src/main.rs` — hands a `PongGame` to `dynamo_lib::start`.
